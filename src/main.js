...
const updateChart = filteredResources => {
    ...
    switch (datasetName) {
        case 'perResource':
            ...
            // Count events per resource on the fly (not efficient for large datasets)
        ...
        case 'perDay':
            ...
            // Count events per day in the active view range
            data = [];
            for (let i = 0; i < days; i++) {
                const
                    from      = DateHelper.add(startDate, i, 'day'),
                ...
                data.push({
                    // Displays Mon, Tue etc. in Day and Week views, and Mon 1, Tue 2 etc. in Month view
                    label :
                        DateHelper.format(from, 'ddd') +
                        (activeView.isMonthView
                            ? ' ' + DateHelper.format(from, ' D')
                            : ''),
